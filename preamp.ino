...
const int encoder0GroundPin = 6;                    //pin 6 is ground
const int encoder0PowerPin = 7;                     //pin 7 is power
int encoder0PinA = 5;                               //encoder pin A
int encoder0PinB = 4;                               //encoder pin B
int encoder0Pos = 0;                                //current encoder value
int encoder0PinALast = HIGH;                        //  ??????????????
int n = LOW;                                        //  ?????
float encoderIncrement = 2;                         //  KAN WEG, waren stappen die hij maakte bij + en - volume
...
int changeVolume(int increment) {
  timeOfLastOperation = millis();
  sleeping = false;
  if (muteEnabled) { changeMute(); }                                        // why is change mute constantly changed 
  int newAttenuatorLevel = currentAttenuatorLevel + increment;              // define new attenuator level
  if not (newAttenuatorLevel <= 63 && newAttenuatorLevel >= 0) {            // check if level is still within range
    if (muteEnabled) { changeMute(); }                                       
    setAttenuatorLevel(newAttenuatorLevel);                                 // schrijf new volume level to the ladder
  } else if (newAttenuatorLevel > 63) {                                     // volume cant be higher as 63
    if (muteEnabled) { changeMute(); }                                      // change mute setting
    setAttenuatorLevel(63);                                                 //
  } else if (newAttenuatorLevel < 0 ) {                                     // volume cant lower as 0
